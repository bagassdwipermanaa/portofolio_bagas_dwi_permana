...
function Welcome({ onLoadingComplete }) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      // Start exit animation after checkmark is fully visible
      setTimeout(() => {
        setShowContent(false);
        // Call parent callback after exit animation
        setTimeout(() => {
          onLoadingComplete && onLoadingComplete();
        }, 800);
      }, 1200); // Increased delay to prevent flickering
    }, 2500);
      ...
      {showContent && (
        ...
        >
          ...
          >
            ...
            >
              {isLoading ? "Preparing portfolio modules…" : "Portfolio ready!"}
            </motion.p>

            {/* Success checkmark animation - positioned absolutely to prevent layout shift */}
              ...
              <AnimatePresence>
                {showCheckmark && (
                  <motion.div
                    ...
                    className="absolute inset-0 flex items-center justify-center text-green-400 text-2xl"
                  >
                    ✓
                  </motion.div>
                )}
